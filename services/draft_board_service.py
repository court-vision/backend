...
Recommendations (v1) rank the available, non-cap-blocked pool by
    ...
    score = vorp + scarcity + flexibility − injury
...
sophistication this season, and marginal category fit is Phase 2.
"""
...
class DraftBoardService:
    ...
    def _recommend(
        ...
        for c in pool:
            ...
            score = round(vorp + scarcity + flexibility + injury, VALUE_DECIMALS)
            scored.append(DraftRecommendation(
                ...
                reason=DraftBoardService._reason(
                    c["name"], position, vorp, scarcity, flexibility, injury, c["injury"]
                ),
            ))
