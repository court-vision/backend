"""Resolve a fantasy provider's player ids to NBA (nba_api) player ids.

Rosters, matchups and free-agent lists are addressed by the provider's own
...
batched query rather than per-player lookups; the adapter's `nba_ids()`
decides which to use and keys the answer the way the provider keys players.

ESPN ids map directly through `nba.players.espn_id`. Yahoo ids have no column
of their own yet (docs/YAHOO_PARITY_PLAN.md P3) and share no namespace with
ESPN's, so Yahoo resolves by normalized name; matching a Yahoo id against
`espn_id` would silently return a different player.
"""
...
def nba_ids_by_name(names: list[str]) -> dict[str, int]:
    ...
    Callers must key lookups with `normalize`, the same normalization the
    stored-stat lookups use.
    """
    if not names:
        return {}
